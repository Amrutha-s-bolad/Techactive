...
<div class="row coredocs-link">
    ...
    <div class="col-md-6 docs-content">
        ...
        <h3 class="coredocs-link-title" id="{{ section_key }}-{{ link_key|slugify }}">{{ link.title|default:link_key }}
            <a href="#{{ section_key }}-{{ link_key|slugify }}"><i aria-hidden="true" class="fa fa-link"></i>
            </a></h3>
        ...
        <table class="parameters table table-bordered table-striped">
            ...
            <tr>
                <td class="parameter-name"><code>{{ field.name }}</code>{% if field.required %} <span
                        class="label label-warning">required</span>{% endif %}
                </td>
